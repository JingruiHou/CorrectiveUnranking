...
def cocol_distil_train(train_iter, disjoint_loader, model_t, model_s, config, test_loader=None, ):
    ...
    def _process_forget_entangled_pair(q, docs):
        if config.get('concatenate_query_doc') and config['concatenate_query_doc']:
    ...
    for epoch in range(config['start_epoch'] + 1, config['start_epoch'] + config['epochs'] + 1):
        ...
        for batch in train_iter:
            ...
            loss = criterion(score_forgetting_t, score_forgetting_s, loss_type='diff_1')
            ...
            if len(q_retaining) > 0 and len(d_retaining) > 0:
                ...
                retaining_loss = criterion(score_retaining_t, score_retaining_s, loss_type='sim_1')
                loss += retaining_loss
            loss.backward()
            optimizer.step()
            step += 1
        ...
        for batch in disjoint_loader:
            ...
            loss_retaining_p = criterion(score_disjoint_neg_t, score_disjoint_neg_s, loss_type='sim_1')
            loss_retaining_n = criterion(score_disjoint_pos_t, score_disjoint_pos_s, loss_type='sim_1')
