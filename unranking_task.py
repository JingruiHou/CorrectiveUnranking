...
class UnRankingTask:
    ...
    def load_model(self, load_state=True, epoch=1, freezing_parameters=False):
        ...
        if load_state:
            ...
            if not os.path.exists(saved_model_path_stage):
                return None
    ...
    def do_test(self, test='test', epoch=1):
        ...
        if not model:
            print(f'model does not exits, pass...')
            return
        if test == 'dev':
    ...
    def do_neg_gradient_retrain(self):
        forget_dataset, substitute_dataset = UnRankingDataConstructor(
    ...
    def do_CoCoL_retrain(self):
        ...
        for x in range(self.config['unlearn_epoch']):
            ...
            self.model = train_ranking_model(substitute_loader, self.model, self.config)
            self.config['model_save_step'] = 30
...
class UnRankingDataConstructor:
    ...
    def build_amnesiac_dataset(self):
        ...
        amnesiac_dataset = SimpleStringDataset(forget_queries + substitute_queries,
                                               forget_neg_documents + substitute_pos_documents,
                                               forget_pos_documents + substitute_neg_documents)
    ...
    def build_cocol_dataset(self):
        ...
        for q, pos, _q, _pos in forget_entangled_list:
            ...
            if _q and _pos:
                _qid, _did, _q_txt, _pos_txt = dataset[idx]
            else:
                _q_txt, _pos_txt = "", ""
